...
const update = (task_Name: string, task_Description: string, due_Date: Date, id: number) => Query('update tasks set task_name = ?, task_description = ?, due_date = ? where id = ?', [task_Name, task_Description, due_Date, id]);
